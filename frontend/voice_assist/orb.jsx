...
function Orb({ state = 'idle', size = 380 }) {
  ...
  const amp = ({
    ...
    connecting: 0.18 + 0.10 * Math.sin(t * 1.6),
    listening:  0.30 + 0.22 * (0.5 + 0.5 * Math.sin(t * 1.2)) * (0.6 + 0.4 * Math.sin(t * 3.1 + 1.2)),
    thinking:   0.22 + 0.10 * Math.sin(t * 0.6),
    speaking:   0.55 + 0.30 * (0.5 + 0.5 * Math.sin(t * 2.6 + Math.sin(t * 1.3) * 1.5)),
  })[state];
  ...
  // Particles (thinking)
  const particles = React.useMemo(() => Array.from({length: 12}).map((_, i) => ({
  ...
  return (
    <div className={`orb orb-${state}`} style={{ width: size, height: size }}>
      ...
      {(state === 'listening' || state === 'idle') && (
        <React.Fragment>
          <div className="orb-pulse-ring" style={{ animationDuration: state === 'listening' ? '2.2s' : '4s' }} />
          <div className="orb-pulse-ring delay-2" style={{ animationDuration: state === 'listening' ? '2.2s' : '4s' }} />
          <div className="orb-pulse-ring delay-3" style={{ animationDuration: state === 'listening' ? '2.2s' : '4s' }} />
        </React.Fragment>
      )}
      ...
      {state === 'connecting' && (
        <svg className="orb-spinner" viewBox="0 0 240 240">
          <circle cx="120" cy="120" r="108" fill="none"
                  stroke="var(--bio-indigo)" strokeWidth="1.2"
                  strokeDasharray="2 8" opacity="0.7" />
          <circle cx="120" cy="120" r="100" fill="none"
                  stroke="var(--bio-cyan)" strokeWidth="0.8"
                  strokeDasharray="40 220" opacity="0.9" />
        </svg>
      )}
      ...
      {/* Mid ring */}
      <svg className="orb-mid-ring" viewBox="0 0 240 240"
        ...
        <circle cx="120" cy="120" r="98" fill="none"
                stroke="url(#ringGrad)" strokeWidth="1.5" />
        <circle cx="120" cy="120" r="86" fill="none"
                stroke="rgba(165,176,251,0.25)" strokeWidth="0.6" />
      </svg>
        ...
        <svg className="orb-listen-wave" viewBox="0 0 240 60" preserveAspectRatio="none">
          {[0, 1, 2].map(layer => (
            <path key={layer}
                  ...
                  fill="none"
                  stroke={layer === 0 ? 'var(--bio-indigo)' : layer === 1 ? 'var(--text-bioluminescent)' : 'var(--bio-cyan)'}
...
function listenPath(t, amp) {
  ...
  for (let i = 0; i <= 48; i++) {
    ...
    const y = H/2 + Math.sin(k * 12 + t * 2.2) * 14 * amp * env
